...
type ImageStorageConfig struct {
	...
	MaxDownloadBytes int64  `mapstructure:"max_download_bytes"`   // 下载上游 url 图片的字节上限
}

// IsConfigured 检查对象存储必要字段是否已配置。
func (c *ImageStorageConfig) IsConfigured() bool {
	return c != nil && strings.TrimSpace(c.Bucket) != "" && strings.TrimSpace(c.AccessKeyID) != "" && strings.TrimSpace(c.SecretAccessKey) != ""
}
