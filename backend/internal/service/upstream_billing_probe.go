...
const (
	// 这些值保存在 accounts.extra 中，因此无需新增数据库迁移。
...
// UpstreamBillingProbeMaxBatchSize 限制单次手动批量探测和单轮定时任务的账号数。
...
// UpstreamBillingProbeSettings 控制定时探测任务。
type UpstreamBillingProbeSettings struct {
	Enabled         bool `json:"enabled"`
	IntervalMinutes int  `json:"interval_minutes"`
}

// UpstreamBillingProbeSnapshot 持久化在 accounts.extra 中。
// Data 使用清理后的 map 保存，后续响应字段扩展无需修改数据库结构。
...
// UpstreamBillingProbeResult 是手动探测接口返回的结果。
...
// GetUpstreamBillingProbeSettings 在配置缺失时返回默认值。
...
// SetUpstreamBillingProbeSettings 校验并持久化定时探测配置。
...
// UpstreamBillingProbeService 探测远端 TokenRouter 兼容实例的计费倍率快照。
...
// ProvideUpstreamBillingProbeService 启动进程级定时探测任务。
...
func (s *UpstreamBillingProbeService) listDueAccounts(ctx context.Context, now time.Time) ([]Account, error) {
	...
	// 非生产仓储及旧适配器继续使用通用查询路径，任务仍会在发起网络请求前截断结果。
...
// ProbeAccount 执行一次手动探测；手动调用不受全局和账号开关限制。
...
// ProbeAccounts 执行有界的手动批量探测，并与定时任务共用并发上限。
...
func nextProbeDelay(intervalMinutes, failureCount int, retryAfterDuration time.Duration) time.Duration {
	...
	if retryAfterDuration > interval {
		// Retry-After 是上游的明确指令，不能用本地指数退避上限缩短它。
