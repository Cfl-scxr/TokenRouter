...
func TestImageTaskServiceOversizedResultBecomesFailed(t *testing.T) {
	store := &imageTaskMemoryStore{}
	svc := NewImageTaskServiceWithOptions(store, time.Hour, time.Minute)
	...
	result := json.RawMessage(`{"data":[{"url":"https://example.test/image.png","metadata":"` + strings.Repeat("x", defaultImageTaskMaxResultBytes) + `"}]}`)
	require.NoError(t, svc.Complete(context.Background(), created.ID, http.StatusOK, result))
	...
	require.Equal(t, ImageTaskStatusFailed, got.Status)
	require.Equal(t, http.StatusBadGateway, got.HTTPStatus)
	require.Contains(t, string(got.Error), "storage limit")
	require.Empty(t, got.Result)
}

func TestImageTaskServiceOversizedErrorIsReplaced(t *testing.T) {
	store := &imageTaskMemoryStore{}
	svc := NewImageTaskServiceWithOptions(store, time.Hour, time.Minute)
	owner := ImageTaskOwner{UserID: 1, APIKeyID: 2}
	created, err := svc.Create(context.Background(), owner)
	...
	taskErr := json.RawMessage(`{"type":"api_error","message":"` + strings.Repeat("x", defaultImageTaskMaxErrorBytes) + `"}`)
	require.NoError(t, svc.Fail(context.Background(), created.ID, http.StatusBadGateway, taskErr))

	got, err := svc.Get(context.Background(), owner, created.ID)
	require.NoError(t, err)
	require.Equal(t, ImageTaskStatusFailed, got.Status)
	require.Less(t, len(got.Error), defaultImageTaskMaxErrorBytes)
	require.Contains(t, string(got.Error), "image generation failed")
}
