...
func TestSystemPlatformQuotas_WriteReadRoundTrip(t *testing.T) {
	...
	// 平台补齐契约：无论写了几个 platform，读回必须含全部平台。
	for _, p := range AllowedQuotaPlatforms {
		if _, ok := got[p]; !ok {
			t.Errorf("platform contract violated: missing platform %q", p)
		}
	}
	// 写入值正确往返
...
// 写入 DefaultPlatformQuotas={} 后，GetDefaultPlatformQuotas 返回全部平台、所有字段均为 nil，
...
func TestSystemPlatformQuotas_EmptyMapClearsAll(t *testing.T) {
	...
	for _, p := range AllowedQuotaPlatforms {
		if _, ok := got[p]; !ok {
			t.Errorf("platform contract violated after empty write: missing %q", p)
		}
	}
	// 所有字段 nil（全部已清空）
	for _, p := range AllowedQuotaPlatforms {
