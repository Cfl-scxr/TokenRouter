...
// pngBytes 是能被 http.DetectContentType 识别为 image/png 的最小测试数据。
...
func TestImageResultUploaderRewritesURL(t *testing.T) {
	storage := &fakeImageStorage{}
	client := &http.Client{Transport: imageDownloadRoundTripper(func(req *http.Request) (*http.Response, error) {
...
func TestImageResultUploaderRejectsPrivateImageURL(t *testing.T) {
	...
	_, err := uploader.Rewrite(context.Background(), "imgtask_private", json.RawMessage(`{"data":[{"url":"http://127.0.0.1/internal.png"}]}`))
	require.Error(t, err)
...
func TestImageResultUploaderRejectsOversizedB64BeforeUpload(t *testing.T) {
	storage := &fakeImageStorage{}
	uploader := NewImageResultUploader(storage, "images/", 8, nil)
	b64 := base64.StdEncoding.EncodeToString(pngBytes)

	_, err := uploader.Rewrite(context.Background(), "imgtask_large", json.RawMessage(`{"data":[{"b64_json":"`+b64+`"}]}`))
	require.Error(t, err)
	require.Contains(t, err.Error(), "exceeds 8 bytes")
	require.Empty(t, storage.saved)
}
...
func TestImageResultUploaderRejectsNonImagePayload(t *testing.T) {
	storage := &fakeImageStorage{}
	uploader := NewImageResultUploader(storage, "images/", 0, nil)
	b64 := base64.StdEncoding.EncodeToString([]byte("<html>not an image</html>"))

	_, err := uploader.Rewrite(context.Background(), "imgtask_html", json.RawMessage(`{"data":[{"b64_json":"`+b64+`"}]}`))
	require.Error(t, err)
	require.Contains(t, err.Error(), "unsupported image content type")
	require.Empty(t, storage.saved)
}

func TestImageResultUploaderRejectsUnexpectedResponseShape(t *testing.T) {
	uploader := NewImageResultUploader(&fakeImageStorage{}, "images/", 0, nil)

	_, err := uploader.Rewrite(context.Background(), "imgtask_missing", json.RawMessage(`{"created":1}`))
	require.ErrorContains(t, err, "does not contain data")

	_, err = uploader.Rewrite(context.Background(), "imgtask_empty", json.RawMessage(`{"data":[]}`))
