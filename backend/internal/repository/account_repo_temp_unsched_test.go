...
func TestAccountRepository_ListOAuthRefreshCandidates_SQLFilter(t *testing.T) {
	...
	require.Contains(t, normalized, "(type IN ('oauth', 'setup-token') AND platform IN ('anthropic', 'openai', 'gemini', 'antigravity'))")
	require.Contains(t, normalized, "OR (platform = 'qoder' AND type = 'cosy')")
