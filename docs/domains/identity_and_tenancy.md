...
邮箱域名白名单为空时，普通注册和 OAuth 邮箱补全允许所有域名。白名单非空时，白名单域名不限注册数量；其它邮箱按公共后缀规则归一为可注册主域名（eTLD+1），每个主域名只允许一个未删除用户，子域名共享额度。验证码发送前会预检额度，最终创建仍须在注册事务内持有主域名锁并复查，避免并发穿透；当前用户邮箱绑定/换绑与已验证邮箱 OAuth 自动建号保持严格白名单语义，不使用该额度放宽。
