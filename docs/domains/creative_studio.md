...
- `openai`：`generate` 走 `/v1/images/generations`（JSON）；`edit`/`inpaint` 走 `/v1/images/edits`（multipart，多源图 + mask）。三条路径都透传尺寸、质量和背景，内部固定 `output_format: "png"`、单张 `n=1`，使用 `response_format: "b64_json"` 取回结果。
