...
管理员模型广场可以在首次进入和缓存过期后复用批量接口读取余额/额度，并在每个分组自己的探针信息区展示；全渠道健康总表不展示这些字段。查询间隔不得短于五分钟，页面原有 30 秒健康刷新不能绕过该门禁，页面隐藏时也不能发起查询。成功结果使用独立的管理员会话缓存；单账号错误、协议不支持或字段缺失不显示对应余额或额度，不能阻断其它渠道和市场主体，也不能把失败结果写成零。折算倍率按分组 `rate_multiplier ×` 账号 `rate_multiplier` 计算，缺失值按 `1`；它是系统配置的计算结果，不是上游声明倍率，本功能不恢复旧的自动倍率探测。
