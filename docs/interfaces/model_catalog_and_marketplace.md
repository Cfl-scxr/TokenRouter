...
可用性窗口和 bucket 粒度来自运行设置，并有日数、分钟数和最大 bucket 数约束，防止公开接口返回过大序列。默认展示近 24 小时、每 15 分钟一格，共 96 格；时间桶使用配置时区，且只用于历史趋势，不参与实时调度和计费。
...
快照只允许通过管理员接口 `GET /api/v1/admin/marketplace/routing-health` 读取。后端使用结构化类型重新序列化，只暴露分组/账号/Key 的展示名、人工启停、综合状态、健康分、真实业务成功率、延迟、连续失败、路由状态、恢复探测和更新时间；源文件中的未知字段不会透传。接口读取失败时返回 `available=false` 的稳定降级响应，不阻断普通模型广场。
