...
升级后，长上下文用户价格只由模型价格、渠道显式区间和分组倍率决定。渠道显式区间优先且不会重复叠加模型内置倍率；没有显式区间时，超过模型阈值的整次会话按模型广场公开的长上下文档结算。账号统计和账号 `quota_used` 统一使用 `COALESCE(account_stats_cost, total_cost) × account_rate_multiplier`，显式零账号成本不累计额度；这不改变用户余额、订阅或 API Key 配额继续使用 `ActualCost` 的规则。
