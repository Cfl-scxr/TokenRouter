...
func (s *GrokQuotaService) fetchBilling(
	...
	if resp.StatusCode >= 400 {
		...
		return nil, resp.StatusCode, infraerrors.Newf(mapUpstreamStatusCode(resp.StatusCode), "GROK_QUOTA_PROBE_UPSTREAM_ERROR", "billing returned %d: %s", resp.StatusCode, bodyText)
	}
