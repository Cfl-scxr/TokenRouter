...
健康总览按最近命中、健康分、探测可用率排序，只影响观察列表；健康分使用数字和进度条，中断可保留长期分。窄容器使用纵向字段布局，保留路由资格、业务成功率和探测时间。单渠道四字段使用容器响应布局，相对时间每三十秒更新，完整时间和历史桶样本通过悬浮查看。历史桶颜色表示成功比例，不能宣称其中包含接口没有提供的逐次延迟。
