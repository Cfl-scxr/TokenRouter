...
4. 创建 Redis 客户端、仓储、服务、handler、中间件和 Gin server。多个 provider 会在构造后立即启动各自 worker，例如 token 刷新、到期处理、调度快照、用量记录、聚合、清理、备份、异步任务和支付订单过期处理。
...
| S3 兼容存储 | 异步图片结果、备份等可选大对象 | 客户端按运行时设置构造，不能在启动时固定旧凭据；对象可用性与数据库/Redis 元数据生命周期必须协同 |
