	...
	return func(c *gin.Context) {
		...
		if status < 400 {
			if parsed.StreamFailure {
				...
				// 仅有孤立的尝试事件且没有网关请求上下文时不落库；这通常是
				// 单元测试或已完成请求留下的诊断信息，不能冒充一次运维错误。
				if len(events) > 0 {
					_, hasModel := c.Get(opsModelKey)
					_, hasAccount := c.Get(opsAccountIDKey)
					if !hasModel && !hasAccount && getOpsAPIKey(c) == nil {
						return
					}
				}
