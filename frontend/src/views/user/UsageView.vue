        ...
        <div class="card p-4">
          <div class="flex items-center gap-3">
            <div class="rounded-lg bg-green-100 p-2 dark:bg-green-900/30">
              <BalanceIcon size="md" class="text-green-600 dark:text-green-400" />
            </div>
            <div class="min-w-0 flex-1">
              ...
              <p class="text-xl font-bold text-green-600 dark:text-green-400">
                {{ formatBalanceAmount(usageStats?.total_actual_cost || 0, { fractionDigits: 4 }) }}
              </p>
              <p class="text-xs text-gray-500 dark:text-gray-400">
                {{ t('usage.actualCost') }} /
                <span class="line-through">{{ formatUsdAmount(usageStats?.total_cost || 0, { fractionDigits: 4 }) }}</span>
          ...
          <template #cell-cost="{ row }">
            <div class="flex items-center gap-1.5 text-sm">
              <span class="font-medium text-green-600 dark:text-green-400">
                {{ formatBalanceAmount(row.actual_cost ?? 0, { fractionDigits: 6 }) }}
              </span>
              <!-- Cost Detail Tooltip -->
              <div
                class="group relative"
        ...
        <div class="space-y-1.5">
          ...
          <div class="mb-2 border-b border-gray-700 pb-1.5">
            ...
            <div v-if="tooltipData && tooltipData.input_cost > 0" class="flex items-center justify-between gap-4">
              ...
              <span class="font-medium text-white">{{ formatUsdAmount(tooltipData.input_cost, { fractionDigits: 6 }) }}</span>
            </div>
            ...
            <div v-if="tooltipData && tooltipData.cache_creation_cost > 0" class="flex items-center justify-between gap-4">
              ...
              <span class="font-medium text-white">{{ formatUsdAmount(tooltipData.cache_creation_cost, { fractionDigits: 6 }) }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.cache_read_cost > 0" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('admin.usage.cacheReadCost') }}</span>
              <span class="font-medium text-white">{{ formatUsdAmount(tooltipData.cache_read_cost, { fractionDigits: 6 }) }}</span>
            </div>
          </div>
          <!-- Rate and Summary -->
          ...
          <div class="flex items-center justify-between gap-6">
            ...
            <span class="font-medium text-white">{{ formatUsdAmount(tooltipData?.total_cost, { fractionDigits: 6, fallback: `${usdUnitSymbol}0.000000` }) }}</span>
          </div>
          <div class="flex items-center justify-between gap-6 border-t border-gray-700 pt-1.5">
            <span class="text-gray-400">{{ t('usage.billed') }}</span>
            <span class="font-semibold text-green-400"
              >{{ formatBalanceAmount(tooltipData?.actual_cost, { fractionDigits: 6, fallback: `${balanceUnitSymbol}0.000000` }) }}</span
            >
          </div>
        </div>
        <!-- Tooltip Arrow (left side) -->
