...
- [可观测性与数据生命周期](observability_and_data_lifecycle.md)：日志、Ops、Usage、审计、聚合、清理、备份和数据共享的数据面总览与专题路由。读取时机：判断数据所有权、留存、备份范围或进入详细观测专题前读取。
- [账号维护](account_maintenance.md)：凭据刷新、临时不可调度、账号测试、自动恢复、额度和能力探测。读取时机：修改 token refresh、账号状态、计划测试、quota/billing probe 或恢复策略时读取。
...
- [数据共享](data_sharing.md)：分组显式采集、异步 worker/buffer、会话质量、票据、导出产物和隐私边界。读取时机：修改数据共享开关、采集、质量、存储、导出或用户告知时读取。
