...
// 中的位置，确保所有 INSERT 列表保持同步；requested_reasoning_effort 位于末尾。
func TestPrepareUsageLogInsert_SessionIDArgWiring(t *testing.T) {
	require.Len(t, usageLogInsertArgTypes, 63, "arg-type table must include team attribution and requested effort")
	...
	// session_id 位于 created_at 之前，requested_reasoning_effort 追加在末尾。
...
func TestPrepareUsageLogInsert_RequestedReasoningEffortArgWiring(t *testing.T) {
	...
	value, ok := prepared.args[len(prepared.args)-1].(sql.NullString)
	require.True(t, ok)
